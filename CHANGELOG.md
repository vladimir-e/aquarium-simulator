...
- **Vitality model: plants-as-fish-benefit + FishCard Conditions** - Task 40 PR follow-up: fourth fish benefit factor `Plants` (peak 0.2 %/h, saturating at ~3 full-grown healthy plants via `min(1, Σ (size/100)×(cond/100) / 3.0)`); raises the planted-tank benefit budget to ≈1.2 %/h on purpose so a healthy planted tank accumulates surplus on `Fish.surplus` — entry point for the future surplus-driven breeding mechanic. `processHealth` / `computeFishVitality` / `calculateStress(Breakdown)` signatures take `plants: Plant[]`; `processLivestock` forwards `state.plants`. FishCard rewritten to mirror PlantCard: merged `▶ Conditions (N)` toggle showing red stressors and green benefits in one block, formatting unified to `+X.XX%/h`; the legacy `STRESSOR_LABELS` lookup is gone — vitality breakdown is the single source of truth.
