...
- **A rescape takes the biofilm with the bed** - any bed swap, from the scape picker or from a preset, costs the colony the share that lived on the old substrate, so fresh soil in an established tank blips before it settles.
- **The biofilter reads as cycled or uncycled** - both toxins at trace on colonies still clearing a real load, so a tank starved for months stops claiming it; share of the surface ceiling is a headroom figure now, not the headline a healthy tank sat at 1 % of.
