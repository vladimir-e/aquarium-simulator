...
- **A tunable is held to the range it declares** - `config set` reads the leaf's own min/max, so a negative attenuation no longer makes the tank's light infinite; the save schema bounds it too.
