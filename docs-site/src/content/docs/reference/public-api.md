...
| Ask before acting | `canDose`, `canAddPlant`, `canAddFish`, `canScrubAlgae`, `canTrimPlants`, `checkFishCapacity`, `getDosePreview`, `getPlantsToTrimCount`, `getMaxPlants`, `getMaxFishMass` | Every refusal an action can make, available before you make it. A UI that asks these never has to guess why a verb is unavailable |
