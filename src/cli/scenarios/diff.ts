...
function split(sample: Sample | undefined): {
  value: number | null;
  grade: Grade | null;
} {
...
function moved(
  ...
  return (
    Math.round(delta * 10 ** digits) >= 1 &&
    delta > MEANINGFUL_SHARE * Math.abs(before)
  );
}

function show(
  { value, grade }: { value: number | null; grade: Grade | null },
  digits: number,
): string {
  if (value === null) return "—";
  return grade === null
    ? value.toFixed(digits)
    : `${value.toFixed(digits)} ${grade}`;
}

export function renderDiff(before: Snapshot, after: Snapshot): string {
  const baseline = new Map(
    Object.entries(before).map(([label, readings]) => [
      setupName(label),
      readings,
    ]),
  );
  const groups = Object.entries(after).flatMap(([label, readings]) => {
    ...
    const lines = READINGS.flatMap((reading) => {
      ...
      const changes = Object.entries(readings[reading.id] ?? {}).flatMap(
        ([day, sample]) => {
          ...
          if (
            from.grade === to.grade &&
            !moved(from.value, to.value, reading.digits)
          )
            return [];
          return [
            `${day} ${show(from, reading.digits)} → ${show(to, reading.digits)}`,
          ];
        },
      );
      return changes.length === 0
        ? []
        : [`  ${reading.label.padEnd(14)} ${changes.join("   ")}`];
    });
