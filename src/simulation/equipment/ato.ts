 ...
 * - Tap water brings its hardness, so topping off evaporation creeps KH and GH up
...
export function atoUpdate(state: SimulationState): Effect[] {
  ...
  const tapKhMass = getKhMass(state.environment.tapKh, waterToAdd);
  const tapGhMass = getGhMass(state.environment.tapGh, waterToAdd);
