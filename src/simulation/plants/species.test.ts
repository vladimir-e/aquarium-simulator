...
describe('getSaturationIrradiance', () => {
  it('reads the roster the published macrophyte ranges expect', () => {
    ...
    expect(getSaturationIrradiance('anubias')).toBe(16);
    expect(getSaturationIrradiance('java_fern')).toBe(20);
    expect(getSaturationIrradiance('amazon_sword')).toBe(40);
    expect(getSaturationIrradiance('dwarf_hairgrass')).toBe(50);
    expect(getSaturationIrradiance('monte_carlo')).toBe(60);
  });
  ...
  it('orders the roster the way the bands do', () => {
    ...
    for (const one of species) {
      for (const other of species) {
        ...
        if (closer) {
          expect(getSaturationIrradiance(one)).toBeLessThan(getSaturationIrradiance(other));
        }
      }
    }
  });
