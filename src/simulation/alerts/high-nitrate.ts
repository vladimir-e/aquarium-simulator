...
/** Threshold for high nitrate alert in ppm (action needed) */
export const HIGH_NITRATE_THRESHOLD_PPM = 20;

export const highNitrateAlert: Alert = {
  id: 'high-nitrate',

  check(state: SimulationState): AlertResult {
    const nitratePpm = gramsToPpm(
    ...
    const isAboveThreshold = nitratePpm > HIGH_NITRATE_THRESHOLD_PPM;
      ...
      if (!wasTriggered) {
        ...
        return {
          log: createLog(
            ...
            'nitrogen',
            'warning',
            `High nitrate level: ${nitratePpm.toFixed(1)} ppm - perform water change or add plants to consume nitrate.`
          ),
